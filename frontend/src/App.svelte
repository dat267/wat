<script lang="ts">
	import { LayoutDashboard, HardDrive, Network, Settings as SettingsIcon, Bot, Terminal } from '@lucide/svelte';
	...
	import ScriptConsole from './components/ScriptConsole.svelte';
	import ToastContainer from './components/ToastContainer.svelte';
	import { initTheme } from './theme.svelte';
	type Tab = 'dashboard' | 'tools' | 'ports' | 'ai' | 'scripts' | 'settings';
	...
	<aside class="w-60 bg-slate-900/10 backdrop-blur-3xl border-r border-white/[0.04] flex flex-col p-6 z-10 justify-between">
		<div class="space-y-8">
			...
			<nav class="flex flex-col gap-1">
				...
				<button
					class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-medium cursor-pointer transition-all border text-left w-full {activeTab === 'ai' ? 'text-white bg-white/[0.03] border-white/[0.05] font-semibold' : 'text-slate-300 border-transparent hover:text-white hover:bg-white/[0.01]'}"
				...
				<button
					class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-medium cursor-pointer transition-all border text-left w-full {activeTab === 'scripts' ? 'text-white bg-white/[0.03] border-white/[0.05] font-semibold' : 'text-slate-300 border-transparent hover:text-white hover:bg-white/[0.01]'}"
		...
		<div class="text-[9px] text-slate-500 pt-4 border-t border-white/[0.03] flex justify-between tracking-widest font-mono">
			<span>v1.1.0</span>
			<span class="text-blue-500/80 font-bold">CONNECTED</span>
		</div>
	</aside>
	<main class="flex-1 h-full overflow-y-auto p-8 bg-gradient-to-b from-slate-950 via-slate-950 to-slate-950/95 relative">
